...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const productsPromise = axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/getBids`);
      const activeProductsPromise = axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/active-products`);
      ...
      if (activeProducts.length > 1) {
        const topDealResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/top-deal`);
        const topDeal = topDealResponse.data.topDeal;
        console.log('Top Deal:', topDeal);

        // Fetch image for top deal
        if (topDeal.imageUrl) {
          const imageResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/images/${topDeal.imageUrl}`);
        ...
        if (singleActiveProduct.imageUrl) {
          const imageResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/images/${singleActiveProduct.imageUrl}`);
          const data = await imageResponse.json();
          const base64String = Buffer.from(data.buffer.data).toString('base64');
          const image = `data:${data.contentType};base64,${base64String}`;
          setImageStream(image);
  ...
  const sendEmailToWinner = async (productName, winningBid, productId) => {
    try {
        const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/sendEmailToWinner`, {
            productName,
            winningBid,
            productId
        });
  ...
  const placeBid = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/placeBid`, {
      ...
      if (response.status === 200) {
        ...
        const updatedUserBidsResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/getUserBids/${userId}`);
  ...
  const handleConfirmModifyBid = async (productId, newBid) => {
    try {
      const res = await axios.put(`https://bidwiser-backend.onrender.com/api/modifyBid/${productId}`, {
        newBid,
      });
      ...
      if (res.status === 200) {
        ...
        const updatedUserBidsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getUserBids/${userId}`);
        setUserBids(updatedUserBidsResponse.data.userBids);
      } else {
