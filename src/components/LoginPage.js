...
const LoginPage = ({ darkMode, userType, setUserType }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('https://bidwiser-backend.onrender.com/api/login', {
        email,
        password
      });
      ...
      if (response.data.message === 'Login successful') {
        ...
        const userBidsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getUserBids/${email}`);
...
const handleForgotPassword = async () => {
  try {
    ...
    const response = await axios.post('https://bidwiser-backend.onrender.com/api/forgotPassword', { email });
