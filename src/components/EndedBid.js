...
export default function EndedBid({product,sendEmailToWinner,handleBid,calculateRemainingTime}){
  const [imageStream,setImageStream] =useState(null)

  useEffect(()=>{
    (async ()=>{const imageResponse =  await fetch(`https://bidwiser-backend.onrender.com/api/images/${product.imageUrl}`);
