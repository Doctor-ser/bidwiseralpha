    ...
    const fetchSellerData = async () => {
      try {
        const [ratingsResponse, productsResponse] = await Promise.all([
          axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/userratings/${userId}`),
          axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/getProductFeedback/${userId}`)
        ]);
...
const ProductCard = ({ product }) => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const imageResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://bidwiser-backend.onrender.com'}/api/images/${product.imageUrls}`);
