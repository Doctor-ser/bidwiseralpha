...
const BiddingPage = ({ darkMode }) => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const response = await axios.get(`https://bidwiser-backend.onrender.com/api/getBids?userId=${userId}`);
  ...
  const handleImageProduct = async (imageUrl) => {
    ...
    try {
      ...
      const response = await axios.post('https://bidwiser-backend.onrender.com/api/upload', formData, {
        headers: {
  ...
  const handleAddProduct = async () => {
    if (loggedIn) {
      ...
      try {
        ...
        const response = await axios.post('https://bidwiser-backend.onrender.com/api/addBid', {
          ...newProduct,
          userId,
  ...
  const handleDeleteProduct = async (productId) => {
    if (loggedIn) {
        try {
            const response = await axios.delete(`https://bidwiser-backend.onrender.com/api/deleteBid/${productId}`);
  ...
  const handleConfirmModifyBid = (productId, newBid) => {
    axios
      .put(`https://bidwiser-backend.onrender.com/api/modifyBid/${productId}`, {
        newBid,
