...
function Chat({ loadMessage, setLoadMessage }) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const [messagesResponse, productResponse] = await Promise.all([
        fetch(`https://bidwiser-backend.onrender.com/api/get-messages?productId=${proid}`),
        fetch(`https://bidwiser-backend.onrender.com/api/fetchsellerbyproid/${proid}`)
      ]);
  ...
  const checkSeller = async () => {
    try {
      const productResponse = await fetch(`https://bidwiser-backend.onrender.com/api/fetchsellerbyproid/${proid}`);
  ...
  const sendMessage = async () => {
    if (newMessage.trim()) {
      ...
      try {
        const response = await fetch('https://bidwiser-backend.onrender.com/api/send-message', {
          method: 'POST',
...
const MessageItem = ({ message }) => {
  ...
  useEffect(() => {
    const fetchSenderName = async () => {
      try {
        const response = await fetch(`https://bidwiser-backend.onrender.com/api/fetchchatusername/${message.senderEmail}`);
        if (response.ok) {
