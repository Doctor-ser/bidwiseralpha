...
const ProductDetails = ({bidChange}) => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        const response = await axios.get(`https://bidwiseralpha.onrender.com/api/products/${productId}`);
        const imageResponse = await fetch(`https://bidwiseralpha.onrender.com/api/images/${response.data.product.imageUrl}`);
        ...
        if (response.data.product.endTime) {
          ...
          if (ended) {
            try {
              const winnerResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/products/${productId}/winner`);
        ...
        try {
          ...
          const response1 = await axios.post(`https://bidwiseralpha.onrender.com/api/products/by-category`, { category, productId });
  ...
  const placeBid = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://bidwiseralpha.onrender.com/api/placeBid', {
        productId,
        userId,
