...
const ChangePasswordForm = ({ userId, onClose }) => {
  ...
  const handleChangePassword = async () => {
    try {
      ...
      const response = await axios.post('https://bidwiser-backend.onrender.com/api/changePassword', {
        userId: userId,
...
const handleCloseChangePasswordForm = () => {
 ...
 useEffect(() => {
   const fetchProfileStatistics = async () => {
     try {
      const topCategoriesResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/top-categories/${userId}`);
      setTopCategories(topCategoriesResponse.data.topCategories);

      const userResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getUserByEmail/${userId}`);
      setUsername(userResponse.data.username);

       const bidsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getTotalBids/${userId}`);
       setTotalBids(bidsResponse.data.totalBids);

       const productsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getTotalProducts/${userId}`);
       setTotalProducts(productsResponse.data.totalProducts);

       const winningBidsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getWinningBids/${userId}`);
       setWinningBids(winningBidsResponse.data.winningBids);


       const userBidsResponse = await axios.get(`https://bidwiser-backend.onrender.com/api/getUserBids/${userId}`);
