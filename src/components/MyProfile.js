...
const handleCloseChangePasswordForm = () => {
 ...
 useEffect(() => {
   const fetchProfileStatistics = async () => {
     try {
      const topCategoriesResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/top-categories/${userId}`);
      setTopCategories(topCategoriesResponse.data.topCategories);

      const userResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/getUserByEmail/${userId}`);
      setUsername(userResponse.data.username);

       const bidsResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/getTotalBids/${userId}`);
       setTotalBids(bidsResponse.data.totalBids);

       const productsResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/getTotalProducts/${userId}`);
       setTotalProducts(productsResponse.data.totalProducts);

       const winningBidsResponse = await axios.get(`https://bidwiseralpha.onrender.com/api/getWinningBids/${userId}`);
