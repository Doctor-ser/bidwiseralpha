...
const ProductsPage = ({ darkMode, email, bidChange }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://bidwiser-backend.onrender.com/api/getBids');
...
const ProductImage = ({ product }) => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const imageResponse = await fetch(`https://bidwiser-backend.onrender.com/api/images/${product.imageUrl}`);
